...
router.post('/pnj/generate', (req,res)=>{
  ...
  res.json({ ok:true, pnj:{ name: randName(), role, level, world, traits: rand(['prudent','observateur','loyal']), hooks: rand(['doit une faveur','cherche un artefact','craint une rumeur']), image: 'https://picsum.photos/seed/pnj'+Math.floor(Math.random()*999)+'/512/768' } });
});
