...
router.post('/quest/generate', (req,res)=>{
  ...
  const quest = { title: (size==='one-shot'?'L\'echo des ruines':(size==='short'?'La cite sous les marais':(size==='long'?'Les anneaux de braise':'Les etoiles muettes'))), bbeg:{ name:'L\'Onde-Noire', motive:'briser le sceau ancien', flaw:'arrogance rituelle' }, acts: Array.from({length:acts}).map((_,i)=>({act:i+1, goal:'Objectif '+(i+1), twist: (i%2?'allie douteux':'piege ancien')})), world, party:{size:partySize, level:partyLevel} };
