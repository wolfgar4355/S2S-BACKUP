...
<body>
  <div class="topbar">
    <div class="row" style="gap:10px">
      ...
      <a href="/toc" class="btn">📑 Table des matières</a>
      <a href="/world/space-opera" class="btn">🚀 Space Opera</a>
    </div>
    <div id="apiChip" class="chip">API: checking…</div>
  </div>
  <div class="container">
    ${body}
  </div>
...
alert('Generated ✔');
      ...
      if (descField) {
        ...
        const theme = data.theme || data.type || data.cat || 'adventure';
          descField.value = base + \n            '[[Mock ' + kind + ']] ' + stamp + '
' + \n            'Party: ' + party + ' • Level/Tier: ' + lvl + '
' + \n            'Theme: ' + theme + '
' + \n            'Synopsis: a neutral, system-agnostic ' + kind + ' suggestion.';
      }
      if (titleField && !titleField.value) {
        titleField.value = (data.title || data.name || kind) + ' — ' + (Date.now() % 10000);
      }
      alert('Generated (local mock) ✔');
    }
  });
});
</script>
    ...
    <div class="book">
      <div class="cover">
        ...
        <form class="login" onsubmit="event.preventDefault(); location.href='/toc'">
          ...
          <div class="note">Démo locale — pas d’envoi réseau.</div>
        </form>
      </div>
    </div>
    `
  ...
  return layout({
    ...
    body: `
      <div class="paper margins">
        ...
        <h2 style="margin:6px 0 10px">Table des matières</h2>
        <div class="list">
          <a href="/world/fantasy"><span>🧙 Fantasy</span><span>→</span></a>
          <a href="/world/space-opera"><span>🚀 Space Opera</span><span>→</span></a>
          <a href="/world/noir"><span>🕵️ Noir</span><span>→</span></a>
          <a href="/world/post-apoc"><span>☢️ Post-Apocalypse</span><span>→</span></a>
          <a href="/world/cyberpunk"><span>🦾 Cyberpunk</span><span>→</span></a>
        </div>
      </div>
    `
  });
  ...
  return `
    ...
    function load(){
      ...
      items.forEach((it,idx)=>{
        ...
        a.innerHTML = '<span>'+ name +'</span><span>🗑️</span>';
...
function generatorWorld(){
  return layout({
    title:'Space Opera — World Generator',
    body: `
      <div class="paper margins">
        <h2>🌌 Space Opera — World Generator</h2>
        <form id="form">
          ...
          <p class="note">Démo locale (localStorage).</p>
        </form>
...
/* ---------- Space Opera — Character / NPC ---------- */
function generatorCharacter(){
  return layout({
    title:'Space Opera — Character / NPC',
    body: `
      <div class="paper margins">
        <h2>👤 Character / NPC</h2>
        <form id="form">
    ...
    body: `
      <div class="paper margins">
        ...
        <form id="form">
          ...
          <div class="field"><label><b>Text Prompt</b></label><textarea name="prompt" rows="3" placeholder="Neutral, non-branded prompt…"></textarea></div>
          <div class="field"><label><b>Reference Image</b></label><input type="file" name="img" accept="image/*"/><div id="preview" class="preview">No image yet.</div></div>
          <div class="row"><button class="btn" type="submit">Add to the world</button><button class="btn" type="button" id="genBtn">Generate</button><a class="btn" href="/world/space-opera">Back</a></div>
        </form>
        <h3 style="margin-top:16px">Entries</h3><div id="list" class="list"></div>
...
/* ---------- Space Opera — Weapons & Armor ---------- */
function generatorWeaponArmor(){
  return layout({
    title:'Space Opera — Weapons & Armor',
    body: `
      <div class="paper margins">
        <h2>🛡️ Weapons & Armor</h2>
        <form id="form">
          <div class="row">
...
function worldSpaceOpera(){
  return layout({
    title:'Space Opera — Generators',
    body: `
      <div class="paper margins">
        <h2>🚀 Space Opera — Generators</h2>
        <div class="list" style="margin-top:8px">
          <a href="/generator/space-opera/world"><span>🌌 World Generator</span><span>→</span></a>
          <a href="/generator/space-opera/character"><span>👤 Character / NPC</span><span>→</span></a>
          <a href="/generator/space-opera/monster"><span>👾 Monster</span><span>→</span></a>
          <a href="/generator/space-opera/loot"><span>💎 Loot</span><span>→</span></a>
          <a href="/generator/space-opera/quest"><span>📜 Quest</span><span>→</span></a>
          <a href="/generator/space-opera/map"><span>🗺️ Map / Dungeon</span><span>→</span></a>
          <a href="/generator/space-opera/weapon-armor"><span>🛡️ Weapons & Armor</span><span>→</span></a>
        </div>
        <p class="note">Chaque générateur inclut : <b>image upload</b>, <b>text description</b>, <b>Add to the world</b> (démo locale).</p>
      </div>
    `
  });
}
